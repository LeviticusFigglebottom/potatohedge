import type { Quote, OHLCV, Interval, OptionsChain, OptionContract, OptionExpiration } from '../types/market.js';
...
export async function getHistory(
  ...
  const res = await fetch(url, { headers: getHeaders(token), signal: AbortSignal.timeout(FETCH_TIMEOUT) });
  if (!res.ok) {
    const body = await res.text().catch(() => '');
...
export async function getExpirations(symbol: string): Promise<OptionExpiration[]> {
  ...
  const res = await fetch(
    ...
    { headers: getHeaders(token), signal: AbortSignal.timeout(FETCH_TIMEOUT) }
  );
