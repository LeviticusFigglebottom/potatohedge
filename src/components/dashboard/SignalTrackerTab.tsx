...
import {
  Crosshair, RefreshCw, Trash2, X, TrendingUp, TrendingDown, Minus,
  CheckCircle2, XCircle, BarChart3, Clock, ArrowUpDown,
} from 'lucide-react';
import {
  loadSignals, updateSignalPrices, closeSignal, removeSignal,
...
export default function SignalTrackerTab() {
  ...
  // Auto-refresh every 5 minutes for open signals
  useEffect(() => {
    const openSignals = signals.filter(s => !s.closed);
  ...
  const handleSort = (key: SortKey) => {
    if (sortKey === key) {
      ...
      setSortKey(key);
      setSortDir(key === 'trackedAt' ? 'desc' : 'desc');
    }
  };

  // Filter and sort
  const filtered = signals.filter(s => {
    if (viewFilter === 'open') return !s.closed;
    if (viewFilter === 'closed') return s.closed;
    return true;
  });
      ...
      <div className="panel">
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-xs text-text-muted/60 mb-6 max-w-lg">
            ...
            of tracking. Check back 1-2 days later to measure accuracy.
          </p>
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex items-center gap-3 px-1">
        <div className="flex items-center gap-1 bg-bg-tertiary rounded-md p-0.5">
          {(['open', 'closed', 'all'] as ViewFilter[]).map(f => (
            <button
              key={f}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border/30">
                  ...
                  <th className="px-3 py-2 text-left text-xs font-mono text-text-muted w-16"></th>
                </tr>
              </thead>
              <tbody>
                {sorted.map((sig, idx) => {
                  ...
                  // For bearish signals, "peak" = most negative move (favorable)
                  const bestMove = sig.bias === 'bearish' ? sig.peakChangePct : sig.peakChangePct;
                  const worstMove = sig.bias === 'bearish' ? sig.troughChangePct : sig.troughChangePct;

                  return (
                    <tr
                      key={sig.id}
                      className={`border-b border-border/10 transition-colors ${
                        sig.closed ? 'opacity-60' : ''
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-xs font-mono">
                        <div className="flex flex-col gap-0.5">
                          {bestMove !== 0 && (
                            <span className={sig.bias === 'bearish' ? 'text-green-400' : 'text-green-400'}>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1">
                          {!sig.closed && (
                            <button
                              onClick={() => handleClose(sig.id)}
                              title="Close signal (stop tracking)"
                              className="p-1 rounded hover:bg-bg-tertiary text-text-muted hover:text-yellow-400 transition-colors"
                            >
      ...
      {sorted.length === 0 && (
        <div className="panel p-8 text-center text-text-muted text-sm font-mono">
          No {viewFilter !== 'all' ? viewFilter : ''} signals to display.
        </div>
      )}
